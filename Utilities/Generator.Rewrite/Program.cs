...
// Modified by Jarle Hansen for use with Kraggs.Graphics.* projects.
//
...
namespace OpenTK.Rewrite
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            if (args.Length == 1)
            {
                if (!File.Exists(args[0]))
                {
                    ...
                    Console.WriteLine("Failed to load file \'{0}\'", Path.Combine(
        ...
        private static void PrintUsage()
        {
            ...
            Console.WriteLine("  Rewrite.exe <work.json>");
        ...
        void Rewrite(clsWorkUnit work)
        {
            ...
            var pdb = Path.ChangeExtension(work.AssemblyExecutable, "pdb");
            var mdb = Path.ChangeExtension(work.AssemblyExecutable, "mdb");
            ...
            if (!String.IsNullOrEmpty(work.AssemblyKeyFile) && File.Exists(work.AssemblyKeyFile))
            {
                work.AssemblyKeyFile = Path.GetFullPath(work.AssemblyKeyFile);
                var fs = new FileStream(work.AssemblyKeyFile, FileMode.Open);
            ...
            var assembly = AssemblyDefinition.ReadAssembly(work.AssemblyExecutable, read_params);
            //var rewritten = assembly.CustomAttributes.FirstOrDefault(a => a.AttributeType.Name == "RewrittenAttribute");
            var rewritten = assembly.CustomAttributes.FirstOrDefault(a => a.AttributeType.Name == work.OpenTKRewrittenAttribute);
            if(rewritten == null || ((bool)(rewritten.ConstructorArguments[0].Value) == false))
            //if (rewritten == null)
            {
                ...
                TypeBindingsBase = assembly.Modules.Select(m => m.GetType(work.OpenTKBindingsBase)).First();

                TypeDefinition RewriteType = null;

                foreach (var module in assembly.Modules)
                {
                    foreach (var type in module.Types)
                    {
                        if(type.Name == work.OpenTKRewrittenAttribute)
                        {
                            RewriteType = type;
                            continue;
                        }
                ...
                if(RewriteType == null)
                {
                    // search references assemblies.
                    // TODO:
                    //var test = assembly.Modules[0].
                    throw new NotImplementedException("Not implemented scanning assembly references for AssemblyRewrittenAttribute type.");
                }

                if (RewriteType != null)
                {
                    var rewrite_constructor = RewriteType.GetConstructors().First();
                    var rewrite_attribute = new CustomAttribute(rewrite_constructor);
                    rewrite_attribute.ConstructorArguments.Add(new CustomAttributeArgument(
                        mscorlib.MainModule.GetType("System.Boolean"), true));
                    //type.Module.Import(mscorlib.MainModule.GetType("System.Boolean")), true));
                    //type.Module.Assembly.CustomAttributes.Add(rewritten);
                    assembly.CustomAttributes.Add(rewrite_attribute);
                }                
            ...
            assembly.Write(work.AssemblyOutput, write_params);
        }

        //void Rewrite(TypeDefinition type, IEnumerable<string> options)
        void Rewrite(TypeDefinition type, clsWorkUnit work)
        {
            ...
            if (entry_points != null)
            {
                ...
                entry_signatures.AddRange(type.Methods
                    .Where(t => t.CustomAttributes.Any(a => a.AttributeType.Name == work.OpenTKOpenGLSlotAttribute)));
                //.Where(t => t.CustomAttributes.Any(a => a.AttributeType.Name == "SlotAttribute")));

                //Rewrite(type, entry_points, entry_signatures, options);
                Rewrite(type, entry_points, entry_signatures, work);

                RemoveNativeSignatures(type, entry_signatures, work);
            }

            //if (type.Name == "RewrittenAttribute")
            //{
            //    //TODO: Change this to be per assembly instead of dependant on detecting owner type.
        ...
        int GetSlot(MethodDefinition signature, clsWorkUnit work)
        {
            var slot_attribute = signature.CustomAttributes
                .FirstOrDefault(a => a.AttributeType.Name == work.OpenTKOpenGLSlotAttribute);
        ...
        void Rewrite(TypeDefinition type, FieldDefinition entry_points,
            List<MethodDefinition> entry_signatures, clsWorkUnit work)
        //List<MethodDefinition> entry_signatures, IEnumerable<string> options)
        {
            ...
            wrapper_signatures.AddRange(type.Methods
                ...
                //.Where(m => m.IsPublic && m.CustomAttributes.Any(a => a.AttributeType.Name == work.OpenTKOpenGLFunctionAttribute)));
                .Where(m => m.CustomAttributes.Any(a => a.AttributeType.Name == work.OpenTKOpenGLFunctionAttribute)));
            ...
            foreach (var wrapper in wrapper_signatures)
            {
                var autogenerated = wrapper.CustomAttributes
                    ...
                    .Where(a => a.AttributeType.Name == work.OpenTKOpenGLFunctionAttribute);
                if (autogenerated.Count() > 0)
                {
                    //
                    // TODO: Handle attriutes created with empty constructor and explicit field set vs constructor with arguments.
                    // TODO: AKA 
                    //       EntryPoint("glGetTextureHandleARB"]
                    //       EntryPoint(FunctionName = "glGetTextureHandleARB")
                    // Both set the field FunctionName to "glGetTextureHandleARB" but
                    // Cecil code to retrive its value is NOT the same!!!
                    var signature_name = string.Empty;
                    var first = autogenerated.First();                    
                    if(first.HasFields)
                        signature_name = (string)first
                        .Fields.First(f => f.Name == work.OpenTKOpenGLFunctionFieldName).Argument.Value;
                    //else if(first.HasProperties)
                    //{
                    //    throw new NotImplementedException("Not implemented picking OpenGL Function Name from Properties");
                    //}
                    else if(first.HasConstructorArguments)
                    {
                        signature_name = (string)first.ConstructorArguments.First().Value;
                    }
                    else
                    {
                        throw new InvalidOperationException(string.Format("Failed to retrive FunctionName/EntryPoint value from {0}.{1}", work.OpenTKOpenGLFunctionAttribute, work.OpenTKOpenGLFunctionFieldName));
                    }
                    //.Fields.First(f => f.Name == "EntryPoint").Argument.Value;
                    var signature = entry_signatures.FirstOrDefault(s => s.Name == signature_name);
                    int slot = GetSlot(signature, work);

                    //ProcessMethod(wrapper, signature, slot, entry_points, options);
                    ProcessMethod(wrapper, signature, slot, entry_points, work);
                }
            }

            RemoveSupportingAttributes(type, work);
            ...
            if (type.NestedTypes.Count > 0)
            {
                foreach (var nested_type in type.NestedTypes)
                {
                    ...
                    Rewrite(nested_type, entry_points, entry_signatures, work);
                }
            }
        }

        void RemoveNativeSignatures(TypeDefinition type, List<MethodDefinition> methods, clsWorkUnit work)
        {
            ...
            foreach (var m in methods.Where(s => GetSlot(s, work) != -1))
            {
                type.Methods.Remove(m);
            }
        }

        void RemoveSupportingAttributes(TypeDefinition type, clsWorkUnit work)
        {
            foreach (var method in type.Methods)
            {
                ...
                {
                    if (attr[i].AttributeType.Name == work.OpenTKOpenGLFunctionAttribute)
                    {
                        attr.RemoveAt(i);
                        i--;
                    }
        ...
        static void ProcessMethod(MethodDefinition wrapper, MethodDefinition native, int slot,
            FieldDefinition entry_points, clsWorkUnit work)
